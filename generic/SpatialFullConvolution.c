...
static int nn_(SpatialFullConvolution_updateOutput)(lua_State *L)
{
  ...
  THOmpTensor_(conv2Dmv)(output, 0.0, 1.0, input, tweight, dH, dW, "F", "C");
...
static int nn_(SpatialFullConvolution_updateGradInput)(lua_State *L)
{
  ...
  /* gradient to input */
  THOmpTensor_(conv2Dmv)(gradInput, 0.0, 1.0, gradOutput, weight, dH, dW, "V", "X");
...
static int nn_(SpatialFullConvolution_accGradParameters)(lua_State *L)
{
  ...
  /* gradient to kernels */
  THOmpTensor_(conv2DRevger)(gradWeight, 1.0, scale, gradOutput, input, dH, dW);
  return 0;
}
